...
class Select extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (this.isControlled) {
      ...
      if (!isEqual(selectedIndex, this.state.selectedIndex)) {
        if (onChange) {
          const { options } = this.state;
          const value = this.state.selectedIndex.map(i => options[i]);
          const fakeEvent = {
            target: { value },
            currentTarget: { value },
          };
  ...
  getTrigerrerLabel() {
    ...
    if (!multiple && selectedIndex.length === 1/* && optionsNode[selectedIndex] !== undefined */) {
      mainContent = (
    ...
    } else if (multiple && selectedIndex.length >= 1) {
      mainContent = (
        <TriggererWrapperWithEllispsisChildren>
          <div
            style={{
          ...
          >
            {
              selectedIndex.map(i => (
                <SelectedBadge>
                  {
                    React.cloneElement(optionsNode[i], {
                      onClick: () => {},
                      style: {
                        ...((optionsNode[i].props || {}).style || {}),
                        whiteSpace: 'nowrap',
                        textOverflow: 'ellipsis',
                        overflow: 'hidden',
                        display: 'flex',
                  ...
                  <UnstyledButton
                    style={{ width: '28px' }}
                    onClick={() => {
                      // this.removeIndexFromSelection(i)
                      console.log('this.removeIndexFromSelection()', i);
                    }}
                  >
                    <Close
                      size={8}
    ...
    return (
      <UnstyledButtonBeta
        inert={(multiple && selectedIndex.length > 0)}
        disabled={disabled}
        ...
        {...((multiple && selectedIndex.length > 0)
          ? {}
          : { onClick: (e, ...r) => {
      ...
      >
        <div style={{ position: 'relative', width: '100%' }}>
          ...
          <div style={styles.trigerrerIcon}>
            <Button
              inert={!multiple || (multiple && selectedIndex.length === 0)}
              type="primary"
              style={{
                width: '42px',
                boxSizing: 'border-box',
                ...
                ...disabledStyle,
                pointerEvents: (
                  !multiple || (multiple && selectedIndex.length === 0)
                    ? 'none'
                    : 'all'
                ),
              }}
              icon={<Arrowdown />}
              onClick={(e, ...r) => {
                console.log('sub trigger button on lcick');
                // if (e && e.stopPropagation) {
                //   e.stopPropagation();
                // }
                this.toggleVisibility(e, ...r);
  ...
  getOptionsItem() {
    ...
    return React.Children.map(children, (child, i) => {
      ...
      console.log('i', i);
      console.log('selectedIndex.includes(i)', selectedIndex.includes(i));
      const isTheOne = this.isControlled
        ? value === this.props.value
        : selectedIndex.includes(i.toString())
      ;
  ...
  clickHandler(e) {
    const { selectedIndex: selectedIndexState } = this.state;
    ...
    if (e.currentTarget && e.currentTarget.dataset && e.currentTarget.dataset.index !== undefined) {
      if (this.isControlled) {
        ...
        this.setState(({ selectedIndex }) => ({
          selectedIndex: [...selectedIndexState, newIndex],
          isOpen: false,
        }), this.focusTrigger);
        this.forceUpdate();
      }
